...
def get_dataset(
    ...
    if dataset_name == "openwebtext-train":
        ...
        train_files = [f for f in files if "train" in f and f.endswith(".gz")]
        val_files = [f for f in files if "validation" in f and f.endswith(".gz")]

        if not train_files:
            raise FileNotFoundError(f"No training file found in {cache_dir}")
        if not val_files:
            raise FileNotFoundError(f"No validation file found in {cache_dir}")

        train_gz_path = os.path.join(cache_dir, train_files[0])
        val_gz_path = os.path.join(cache_dir, val_files[0])

        # Generator to not load all data to the memory
        def read_gzip_lines(path):
            """Generator that yields lines without loading entire file to memory"""
